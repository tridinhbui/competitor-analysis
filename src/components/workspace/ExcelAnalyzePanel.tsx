...
export function ExcelAnalyzePanel() {
  ...
  const canAnalyze = useMemo(() => {
    if (inputMode === "excel") return !!excelFile;
    return script.trim().length >= 120;
  }, [excelFile, inputMode, script]);
  ...
  async function analyze() {
    ...
    try {
      ...
      if (inputMode === "excel") {
        ...
        text = await extractExcelText(excelFile);
      }
      ...
      if (!response.ok || !payload.analysis) throw new Error(payload.error || `HTTP ${response.status}`);
              ...
              <>
                <div
                  ...
                  onDrop={(e) => {
                    e.preventDefault();
                    setDragOver(false);
                    assignExcelFile(e.dataTransfer.files?.[0]);
                  }}
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-slate-900 sm:text-base">
                    {excelFile ? excelFile.name : "Drop workbook here or click to browse"}
                  </p>
                  <p className="max-w-sm text-xs text-slate-500 sm:text-sm">
                    First sheets and rows run in-browser. Large books are truncated for stable analysis—focus the
                    key forecast tabs if needed.
                  </p>
                  <input
                    ref={excelInputRef}
                    type="file"
                    ...
                    className="hidden"
                    onChange={(event) => assignExcelFile(event.target.files?.[0] ?? null)}
                  />
                </div>
                <div className="flex flex-shrink-0 items-center justify-between gap-3 border-t border-slate-100 pt-4">
                  <p className="text-xs text-slate-500">{excelFile ? excelFile.name : "No file selected"}</p>
                  <button
                    type="button"
                    onClick={analyze}
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <div>
                ...
                <ul className="space-y-1.5">
                  {(result.growth ?? []).slice(0, 3).map((x, idx) => (
                    <li key={`g-${idx}`} className="rounded-lg border border-slate-100 bg-slate-50/90 px-3 py-2 text-xs text-slate-700">
                      {x.finding}
                    </li>
                  ))}
                  {(result.profitability ?? []).slice(0, 2).map((x, idx) => (
                    <li key={`p-${idx}`} className="rounded-lg border border-slate-100 bg-slate-50/90 px-3 py-2 text-xs text-slate-700">
                      {x.finding}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {(result.riskAnalysis ?? []).slice(0, 3).map((x, idx) => (
                    <li key={`r-${idx}`} className="rounded-lg border border-slate-100 bg-slate-50/90 px-3 py-2 text-xs text-slate-700">
                      {x.finding}
                    </li>
                  ))}
                  {(result.demandSignals ?? []).slice(0, 2).map((x, idx) => (
                    <li key={`d-${idx}`} className="rounded-lg border border-slate-100 bg-slate-50/90 px-3 py-2 text-xs text-slate-700">
                      {x.finding}
                    </li>
                  ))}
                </ul>
              ...
              <div className="mt-5">
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-100">
                  <table className="min-w-full text-xs">
                    ...
                    <tbody>
                      {(result.extractedMetrics ?? []).slice(0, 12).map((m, idx) => (
                        <tr key={`${m.metric}-${idx}`} className="border-b border-slate-100">
                          <td className="px-3 py-2 text-slate-800">{m.metric}</td>
                          <td className="px-3 py-2 text-slate-900">{m.value}</td>
                          <td className="px-3 py-2 text-slate-600">{m.context}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
