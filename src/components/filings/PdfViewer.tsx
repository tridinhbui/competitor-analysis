...
const METRIC_ALIASES: Record<string, string[]> = {
  ...
  "SG&A Expense":      ["selling, general and administrative", "selling, general and administrative expense", "sg&a", "sga expense", "selling general and administrative", "selling general and administrative expense"],
  "R&D Expense":       ["research and development", "research and development expense", "research & development", "research & development expense", "r&d", "rd expense", "redearch and development"],
  ...
  "Operating CF":      ["operating cash flow", "cash from operations", "cash provided by operating", "net cash provided by operating", "cash generated by operations", "cash generated from operations", "cash generated by operating", "net cash generated from operating", "net cash generated by operating"],
  "Free Cash Flow":    ["free cash flow"],
  "EBITDA":            ["ebitda", "ebitda (calculated", "adjusted ebitda"],
  "Interest Expense":  ["interest expense", "interest expense, net", "interest and debt expense"],
  ...
  "Capital Expenditures": ["additions to property, plant and equipment", "payments to acquire property", "capital expenditures", "purchases of property", "purchase of property", "acquisition of property, plant", "property and equipment additions", "property, plant and equipment, net of disposals"],
  /**
...
function normalizeNumber(v: number): string[] {
  ...
  if (abs < 1000) variants.push(Math.round(abs * 1000).toLocaleString("en-US"));
  const inK = abs / 1000;
...
function matchesNumericTarget(combinedRaw: string, absTarget: number): boolean {
  ...
  if (t >= 1 && t < 1_000_000) {
    if (Math.abs(Math.abs(v) - t) < 0.501) return true;
    if (Math.abs(Math.abs(v) - t * 1000) < 5) return true;
    return false;
  }
...
function scoreRow(row: ReconstructedRow, target: PdfTraceTarget): number {
  ...
  if (score === 0) {
    // Fallback for unlabeled subtotal rows, e.g. "$ 5,042,440  $ 5,171,412" at the
    // bottom of an assets section with no "Total assets" label.  Grant a modest base
    // score only when every token in the row is a pure number/dollar/paren — no text —
    // AND the contiguous-value search finds the target number in those tokens.
    if (
      target.value != null &&
      target.value !== 0 &&
      row.tokens.length >= 1 &&
      row.tokens.length <= 10 &&
      row.tokens.every((t) => /^[\d,.()\s$–—-]*$/.test(t.text))
    ) {
      const vt = pickContiguousValueTokens(row.tokens, target.value);
      if (vt.length > 0) score = 18;
    }
    if (score === 0) return 0;
  }
...
async function searchDocForTarget(
  ...
  const all: TraceCandidate[] = [...seeded];
  for (let p = 1; p <= totalPages; p++) {
    if (hintPage != null && p === hintPage) continue;
    try {
...
export function PdfViewer({ file, fullHeight, traceTarget, onClearTrace }: Props) {
  ...
  useEffect(() => {
    ...
    if (!activeCand || activeCand.page !== currentPage) return;
    ...
    // Always show the whole row for the active trace so the user can see which line item was selected.
